...
class UpdateTask extends Binder {
  ...
  render() {
    const { location, match, taskStore } = this.props;
    const { task, formHelpers } = this.state;

    const selectedTask = taskStore.selected.getItem();

    const isEmpty = !task || !task._id;

    const isFetching = !taskStore.selected.id || taskStore.selected.isFetching;
...
export default withRouter(connect(mapStoreToProps)(UpdateTask));
